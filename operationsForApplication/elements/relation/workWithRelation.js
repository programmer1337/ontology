...
let relationChangeModalName = document.getElementById(
  `relationChangeModalName`)
...
let lookAtConnectedElements = document.getElementById(
  `lookAtConnectedElements`)
...
let closeRelationModifyModal = document.getElementById(
  'closeRelationModifyModal')
...
function saveRelationChanges (currentLinkId) {
  ...
  if (relationChangeModalName.value !== '' && relationChangeModalName.value !==
    'Для изменения направления необходимо изменить имя') {
...
function swapRelationOrientation (currentLinkId) {
  let tempRelationBetween = [], tempRelationConnected = [],
    massiveOfRelation = []
  if (relationChangeModalName.value !== '' && relationChangeModalName.value !==
    'Для изменения направления необходимо изменить имя') {
    ...
    if (massiveOfRelation[currentLinkId].RelationElementsInConnection.length !==
      0) {
...
function workWithConnectedElements (currentLinkId) {
  ...
  addConnection.onclick = function () {
    if (document.querySelectorAll('.selected').length === 1) {
      document.getElementById(
        'connectedElementsModalName').textContent = 'Добавление связей'
      ...
      if (document.querySelectorAll('.selected')[0].parentElement.id ===
        'classList1') {
        secondTable.onclick = function (e) {
  ...
  addConnectionButton.onclick = function () {
    ...
    if (massiveOfConnected[0][firstStrokeID] === undefined ||
      massiveOfConnected[0][firstStrokeID] === null) {
    ...
    firstTable.onclick = function (e) {
      selectMode0(e, currentLinkId, elementsInRelation[0],
        elementsInRelation[1])
    }
    secondTable.onclick = function (e) {
      selectMode0(e, currentLinkId, elementsInRelation[0],
        elementsInRelation[1])
  ...
  deleteConnection.onclick = function () {
    if (document.querySelectorAll('.selected').length === 1) {
      document.getElementById(
        'connectedElementsModalName').textContent = 'Удаление связей'
  ...
  backButton.onclick = function () {
    document.getElementById('connectedElementsModalName').textContent
      = 'Править отношение ' +
      sessionStorage.getItem(`RelationName ${currentLinkId}`)
...
function buildTree (currentLinkId) {
  ...
  addEdge(dataForRelationTree, 'spaceEdge-1', 'spaceNode-1', 'spaceNode-2',
    '#97c2fc')
  addEdge(dataForRelationTree, 'spaceEdge-2', 'spaceNode-2', 'spaceNode-3',
    '#97c2fc')

  if (massiveOfConcept[firstInRelationId].ConceptValues.length > 0) {
    console.log(massiveOfConcept[firstInRelationId].ConceptValues);

    for (let i = 0; i <
    massiveOfConcept[firstInRelationId].ConceptValues.length; i++) {
      if (massiveOfRelation[currentLinkId].RelationElementsInConnection.length !==
        0) {
        let secondElementStorage = massiveOfRelation[currentLinkId].RelationElementsInConnection[0][i]

        if (massiveOfRelation[currentLinkId].RelationElementsInConnection[0].length >
          0) {
          console.log(
            massiveOfRelation[currentLinkId].RelationElementsInConnection[0])

          for (let j = 0; j < secondElementStorage.length; j++) {
            if (spaceNodeNow === 4) {
              addNode(dataForRelationTree, `spaceNode-${spaceNodeNow}`, '',
                '#537bb1', levelNow)
              addEdge(dataForRelationTree, `spaceEdge-${spaceEdgeNow}`,
                `spaceNode-${spaceNodeNow - 1}`, `spaceNode-${spaceNodeNow}`)
            } else {
              addNode(dataForRelationTree, `spaceNode-${spaceNodeNow}`, '',
                '#537bb1', levelNow)
              addEdge(dataForRelationTree, `spaceEdge-${spaceEdgeNow}`,
                `spaceNode-${spaceNodeNow - 2}`, `spaceNode-${spaceNodeNow}`)
            }
            spaceNodeNow++
            spaceEdgeNow++

            let positionOfElement = massiveOfConcept[secondInRelationId].ConceptValues.indexOf(
              secondElementStorage[j])
            newBranch = createBranch(i, positionOfElement, currentLinkId,
              spaceNodeNow, levelNow)
            ...
            if (spaceNodeNow > 5) {
              addEdge(dataForRelationTree, `spaceEdge-${spaceEdgeNow}`,
                `spaceNode-${spaceNodeNow - 3}`, `spaceNode-${spaceNodeNow}`)
            } else {
              addEdge(dataForRelationTree, `spaceEdge-${spaceEdgeNow}`,
                `spaceNode-${spaceNodeNow - 2}`, `spaceNode-${spaceNodeNow}`)
            }
  ...
  let relationNetwork = buildNetwork(
    document.getElementById('relationShowingBlock'), dataForRelationTree,
    options)
...
function addNode (massive, id, label, color, level) {
  if (/[А-ЯЁ]/gi.test(label) || label === '' || label === '𝚲') {
    ...
    let testInCorner = htmlTitle(
      katex.renderToString(label).replaceAll('"', '\''))
  ...
  if (!stroke.classList.contains('selected')) {
    ...
    if (stroke.parentElement.id === 'classList1') {
      ...
      if (massiveOfRelation[currLink].RelationElementsInConnection.length !== 0
        &&
        massiveOfRelation[currLink].RelationElementsInConnection[0].length !==
        0) {
      ...
      selectConnectedElements(secondTable, connectedElementsStore,
        massiveOfConcept[secondElement].ConceptType)
      ...
      if (massiveOfRelation[currLink].RelationElementsInConnection.length !== 0
        &&
        massiveOfRelation[currLink].RelationElementsInConnection[1].length !==
        0) {

        connectedElementsStore = massiveOfRelation[currLink].RelationElementsInConnection[1][selected]
      } else connectedElementsStore = ['error_storage_empty']

      selectConnectedElements(firstTable, connectedElementsStore,
        massiveOfConcept[firstElement].ConceptType)
    }

  } else if (stroke.classList.contains('selected')) {
    stroke.classList = ''

    drawTable(firstElement, firstTable,
      massiveOfConcept[firstElement].ConceptValues)
    drawTable(secondElement, secondTable,
      massiveOfConcept[secondElement].ConceptValues)
  }
}
...
function selectMode1 (countOfElem, e, option) {
  ...
  if (option === 'addOption') {
    if (!stroke.classList.contains('selected') && countOfElem < 1 &&
...
function selectConnectedElements (table, connectedElements, type) {
  if (type === 'Формульный') {
    ...
    for (let i = 0; i < massiveOfSelectedElements.length; i++) {
      if (connectedElements.includes(
        massiveOfSelectedElements[i].firstChild.firstChild.lastChild.textContent)) {
...
function createBranch (
  firstInRelation, secondInRelation, relationId, spaceNodeNumber, level) {
  let massiveOfConcept = JSON.parse(sessionStorage.getItem('testConcept'))
  let massiveOfRelation = JSON.parse(sessionStorage.getItem('testRelation'))
  ...
  addEdge(massiveForBranch,
    `r_${relationId} from:SN${spaceNodeNumber} to:${firstInRelation}:${tempStorageForFirst[firstInRelation]}`,
    `spaceNode-${spaceNodeNumber}`,
    `r_${relationId} Node-${tempStorageForFirst[firstInRelation]}->${tempStorageForSecond[secondInRelation]}`)

  addEdge(massiveForBranch,
    `r_${relationId} from:SN${spaceNodeNumber} to:${secondInRelation}:${tempStorageForSecond[secondInRelation]}`,
    `spaceNode-${spaceNodeNumber}`,
    `r_${relationId} Node-${tempStorageForSecond[secondInRelation]}<-${tempStorageForFirst[firstInRelation]}`)
