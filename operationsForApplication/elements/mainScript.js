...
let closeClassCreationModal = document.getElementById(
...
viewRelation.onclick = function () {
  ...
  outputRelationListIn(relationList)

  relationList.onclick = function (e) {
    let id = e.target.closest('tr').id

    const test = buildTree(id)
    console.log(test);
...
let relationFillModal = document.getElementById('fillRelationModal'),
  fillRelationButton = document.getElementById('fillRelation')

fillRelationButton.addEventListener('click', function () {
  relationFillModal.style.display = 'block'

  let fillRelationList = document.getElementById('fillRelationRelList')
  outputRelationListIn(fillRelationList)
})
...
function isCorrect (className) {
  ...
  for (let i = 0; i < massiveOfConcepts.length; i++) {
    if (className.value.toLowerCase() ===
      massiveOfConcepts[i].ConceptName.toLowerCase()) {
      nameIsReserved = true
      break
    }
  }
...
function outputRelationListIn (nodeList) {
